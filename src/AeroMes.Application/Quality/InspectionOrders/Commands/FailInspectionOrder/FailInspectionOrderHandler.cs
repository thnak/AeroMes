...
public class FailInspectionOrderHandler(
    IInspectionOrderRepository repo,
    IUnitOfWork uow,
    IValidator<FailInspectionOrderCommand> validator)
    : ICommandHandler<FailInspectionOrderCommand, ValidationResult<Unit>>
{
    public async Task<ValidationResult<Unit>> HandleAsync(FailInspectionOrderCommand cmd, CancellationToken ct)
    {
